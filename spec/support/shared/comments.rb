shared_examples_for 'Create comment' do
  ...
  context "Authenticated user tries to leave comment with valid attributes" do
    ...
    it "increase comment count by 1" do
      expect { post :create, id: object_comment, comment: attributes_for(:comment), format: :js }.to change(object.comments, :count).by(1)
    end
  end
  ...
  context "Authenticated user tries to leave comment with invalid attributes" do
    ...
    it "doesnt change comment count" do
      expect { post :create, id: object_comment, comment: attributes_for(:invalid_comment), format: :js }.to_not change(Comment, :count)
    end
  end
  ...
  context 'PrivatePub' do
    it 'publishes new object' do
      ...
      post :create, id: object_comment, comment: attributes_for(:comment), format: :js
    end
  end
end
